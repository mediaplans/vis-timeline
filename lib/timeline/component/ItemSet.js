 ...
 */
  ...
  _create() {
    ...
    this.hammer.on('tap',  this._onTap.bind(this));
  ...
  setOptions(options) {
    if (options) {
      ...
      ['onDropObjectOnItem', 'onAdd', 'onUpdate', 'onRemove', 'onMove', 'onMoving', 'onAddGroup', 'onMoveGroup', 'onRemoveGroup', 'onSelect'].forEach(addCallback);
  ...
  setItems(items) {
    ...
    if (this.itemsData) {
      ...
      const selection = [];
      this.itemsData.forEach((item, id) => {
        if (item.selected) {
          selection.push(id)
        }
      });
  ...
  _onSelectItem(event) {
    ...
    if (item) {
      const itemData = this.itemsData.get(item.id); // get a clone of the data from the dataset
      this.options.onSelect(itemData, itemData => {
        // Nothing to do atm
      });
    }
  }
  ...
  /**
  * Handles user action on single click
  * @param {Event} event
  */
  _onTap(event) {
  const item = this.itemFromTarget(event);
